...
def get_constrained_YCP(L_x, L_y, n_x, n_y):
    ...
    print "linien", ycp.F_L
    v = CreasePatternView(root=init)
    v.configure_traits()

    return fold

folding = get_constrained_YCP(L_x=6.43, L_y=2.2, n_x=2, n_y=4)
...
print "Fold_neu", fold_neu 
F1=np.resize(F,(F[:].size,1)) #liste der Facettenpunkte enstsprechend INFO-Cad liste
print 'F', F1 
#c=

#print 'count', i_2=np.arange(1,F1.size, dtype="i")
#print F[0]cd=np.array[]
for i in range(0,F.size/6):
    cd1= np.where(F == F[i,0])
    cd2= np.where(F == F[i,1])
    cd3= np.where(F == F[i,2])
    
     
    #np.append(cd2, axis=0),np.append(cd3,axis=0)])
#print cd
    #print "cd1", cd1
    #print "cd2",cd2
...
#print hl #dreiecke einer creasline
        cd4_1= cd1[0] == hl[0]
        cd4_2= cd1[0] == hl[1]
        cd4_3= cd2[0] == hl[0]
        cd4_4= cd2[0] == hl[1]
#print cd4_1
        ...
        p= np.hstack((h1,h2,h3,h4)) #punkt im dreieck
        Knoten= d[:]*6+p[:]+1
        print "knoten", Knoten
...
'''print cd1_0[:] == cd2_0[:]
#print "cd3", cd3
print "1", np.extract(cd3, cd1)
print "2", np.extract(cd3, cd2)'''
#K_N=
