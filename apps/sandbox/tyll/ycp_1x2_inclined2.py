...
def gt(X):
    ...
    for i in range(0, len(y)):
        ...
        if y[i] > 0:
            y[i] = y[i] - (+x[i] * ((n - 0.11) / L_x))
    x[2], x[3], x[8] , x[9] = 14.622 , 14.622, 8.032, 21.212
...
def get_constrained_YCP(L_x, L_y, n_x, n_y):
    ...
    lift = Folding(cp=ycp, n_steps=8, dof_constraints=fixed_node + \
                                   ...
                                   planar_left_boundary_y +
                                   planar_right_boundary_y +
...
cp = get_constrained_YCP(L_x=25.72, L_y=9.1, n_x=2, n_y=2)
fold = cp
print fold.x_t[0]
#print cp.L_x
#print cp.X_h[:,:]
