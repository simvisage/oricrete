...
print "n", n
d_v=0.025 #vertikal verschiebung
...
for l1 in range(1,4):
    for l in range(0,F.size/6):
        ...
        if hl.size >= 2:
            ...
            #if h4[0]+h1[0]==3 and h3[0]+h2[0]==2:
            if h4[0]+h1[0]==3 and h3[0]+h2[0]==2:             
            ...
            if h4[0]+h1[0]==3 and h3[0]+h2[0]==3 or h4[0]+h1[0]==3 and h3[0]+h2[0]==1 or h4[0]+h1[0]==1 and h3[0]+h2[0]==1:
...
z=np.column_stack((K_A,Verdrehung,Verdrehung,Verdrehung,Freiheitsgrad,Freiheitsgrad,Freiheitsgrad,Federsteifigkeit,Federsteifigkeit,Federsteifigkeit,Berechnungsart))
np.savetxt("Auflager.txt", z, fmt=('%s',"%s","%s","%s", '%s',"%s","%s","%s", "%s", "%s","%s"), delimiter='\t')
