...
@author: jvanderwoerd
'''
from traits.api import Property, Array
...
corner_slides = [([(cp.N_h[0, 0], 0, L_x), (cp.N_h[0, 0], 1, -L_y * ctrl_u_ratio)], 0),
                 ...
                 ([(cp.N_h[-1, -1], 0, L_x),
                   (cp.N_h[-1, -1], 1, -L_y * ctrl_u_ratio)], 0),
                 ([(cp.N_h[0, -1], 0, L_x),
                   (cp.N_h[0, -1], 1, L_y * ctrl_u_ratio)], 0),
...
face_z_t = CnstrTargetFace(
    F=[r_, s_, 1.0 * t_ * (A * r_ * (1 - r_ / L_x) + B * s_ * (1 - s_ / L_y))])
...
# trying
fixed_v_left_z_lower1 = fix(cp.N_v[0, (1, -2)], [2], -0.00)
fixed_v_left_z_lower2 = fix(cp.N_v[0, (0, -1)], [2], -0.00)

push_left = fix(cp.N_i[0, 2], [2], -0.005)
push_right = fix(cp.N_i[-1, 2], [2], -0.005)

#push_left = fix(cp.N_i[0, (1, -2)], [0], -0.01)
#push_right = fix(cp.N_i[-1, (1, -2)], [0], 0.01)

fold_fine = Folding(source=fold,
                    name='Corrector for modified pattern',
                    goal_function_type='potential_energy',
                    n_steps=4,
                    MAX_ITER=400,
                    #tf_lst=[(face_z_t, cp.N)],
                    dof_constraints=fix( cp.N_v[0, 2], [0], 0.00) + \
                    #                                 fix(n_l_h_corner, [0], v) + \
                    #                                fix(n_r_h_corner, [0], -v) + \
                    #                               fix(n_lr_h_corner, [2]) + \
                    fix(n_fixed_y, [1]) + \
